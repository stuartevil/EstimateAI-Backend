...
from app.schemas.measurement_schema import MeasurementCreate, MeasurementResponse, MeasurementUpdate
...
@router.post("/projects/{project_id}", response_model=APIResponse[MeasurementResponse], status_code=201)
async def create_measurement(
    ...
    measurement = await service.create_measurement(project_id, payload, owner_id=current_user.id)
    return success_response(data=MeasurementResponse.model_validate(measurement), message="Measurement created")


@router.get("/projects/{project_id}", response_model=APIResponse[list[MeasurementResponse]])
...
async def get_measurement(
    ...
    return success_response(data=MeasurementResponse.model_validate(measurement), message="Measurement retrieved")
...
async def update_measurement(
    ...
    measurement = await service.update_measurement(measurement_id, payload, owner_id=current_user.id)
    return success_response(data=MeasurementResponse.model_validate(measurement), message="Measurement updated")
